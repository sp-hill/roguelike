...
"""
An event handler return value which can trigger an action or 
switch active handlers.

If a handler is returned then it will become the active handler
for future events.
If an action is returned it will be attempted and if it's valid
...
class InventoryEventHandler(AskUserEventHandler):
	...
	def on_render(self, console: tcod.Console) -> None:
		...
		and the letter to select each one. Will move to a different position
		based on where the player is located, so the player can always see
		where they are.
		"""
		super().on_render(console)
...
class SelectIndexHandler(AskUserEventHandler):
	"""Handles asknig the user for an index on the map."""
...
class AreaRangedAttackHandler(SelectIndexHandler):
	"""
	Handles targetting an area within a given radius.
	...
	def on_render(self, console: tcod.Console) -> None:
		...
		# Draw a rectangle around the targetted area, so the player
...
class GameOverEventHandler(EventHandler):
	def on_quit(self) -> None:
		...
		if os.path.exists(save_file):
			os.remove(save_file) # Deletes the active save file
